<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
    <div>
        ...
        <input type="text" name="nombre" value="{{ old('nombre', $player->nombre) }}" required class="mt-1 w-full px-3 py-2 bg-gray-700 rounded-md text-white">
    </div>
    <div>
        <label class="block text-sm font-medium text-gray-400">Número</label>
        <input type="number" name="numero" value="{{ old('numero', $player->numero) }}" required min="1" max="99" class="mt-1 w-full px-3 py-2 bg-gray-700 rounded-md text-white">
    </div>
</div>
    ...
    <div>
        ...
        <select name="posicion" required class="mt-1 w-full px-3 py-2 bg-gray-700 rounded-md text-white">
            @foreach(['portero', 'defensa', 'medio', 'delantero'] as $pos)
                <option value="{{ $pos }}" @selected(old('posicion', $player->posicion) == $pos)>{{ ucfirst($pos) }}</option>
            @endforeach
        </select>
    </div>
</div>
...
<div class="border-t border-gray-600 pt-3">
    ...
    <img src="{{ $player->foto_url ?? 'https://placehold.co/100x100/1f2937/FFFFFF?text=JUGADOR' }}" class="w-16 h-16 rounded-full object-cover my-2">
