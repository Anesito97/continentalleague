                ...
                <tbody class="divide-y divide-gray-800 text-sm">
                    @forelse($players as $player)
                        <tr class="hover:bg-gray-700 transition">
                            ...
                            <td class="py-3 px-2 text-center hidden sm:table-cell text-yellow-300">{{ $player->amarillas }}</td>
                            ...
                            <td class="py-3 px-2 text-center">
                                <div class="flex space-x-1 justify-center">
                                    ...
                                    <form method="POST" action="{{ route('players.destroy', $player->id) }}"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded-md text-xs">X</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="py-4 text-center text-gray-500">No hay jugadores registrados en este equipo.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
